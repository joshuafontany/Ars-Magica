---
layout: gruv_book_page_adapter
title: Ars Magica 5th Edition STANDARD Core Rulebook
published: true
permalink: /vault/Ars-Magica/ars-magica-5th-edition-standard-core-rulebook/
vault_nexus_key: ars_magica
vault_book_key: ars-magica-5th-edition-standard-core-rulebook
ars_book_slug: ars-magica-5th-edition-standard-core-rulebook
ars_book_title: Ars Magica 5th Edition STANDARD Core Rulebook
ars_category: core_rulebooks
ars_edition: 5th Edition
ars_source_url: https://www.atlas-games.com/product_tables/AG0205
ars_errata_url: https://www.atlas-games.com/arsmagica/arm5errata
ars_has_errata: true
---
# Ars Magica 5th Edition STANDARD Core Rulebook

Pilot landing page for the first Ars Magica import.

## Status

This title is configured for chapter-level ingestion from `~/Ars-Magica/_books/Ars Magica 5th Edition STANDARD Core Rulebook.pdf`.

## Chapters

- [01 Overview](./01-overview/)
- [02 Character Creation](./02-character-creation/)

## Errata

- [View errata layer](../errata/ars-magica-5th-edition-standard-core-rulebook/)
