---
layout: gruv_book_page_adapter
title: Ars Magica 5th Edition STANDARD Core Rulebook - Overview
published: true
vault_nexus_key: ars_magica
ars_book_slug: ars-magica-5th-edition-standard-core-rulebook
ars_book_title: Ars Magica 5th Edition STANDARD Core Rulebook
ars_category: core_rulebooks
ars_edition: 5th Edition
ars_chapter_number: 1
ars_source_url: https://www.atlas-games.com/arsmagica/openars
ars_errata_url: https://www.atlas-games.com/arsmagica/arm5errata
ars_has_errata: true
---
# Overview

This pilot chapter proves the routing, navigation context, and per-book metadata contract for the Ars Magica vault.

## Ingestion Notes

The final generated text for this chapter will come from the owned PDF source. This placeholder page exists so the site can render the new vault end-to-end before the extraction pipeline is fully automated.
