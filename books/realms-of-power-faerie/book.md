---
layout: gruv_book_page_adapter
title: 'Realms of Power: Faerie'
published: true
permalink: /vault/Ars-Magica/realms-of-power-faerie/
vault_nexus_key: ars_magica
vault_book_key: realms-of-power-faerie
ars_book_slug: realms-of-power-faerie
ars_book_title: 'Realms of Power: Faerie'
ars_category: realms_of_power
ars_edition: 5th Edition
ars_source_url: https://www.atlas-games.com/product_tables/AG0290
ars_errata_url: https://www.atlas-games.com/arm5/arm5errata.php/
ars_has_errata: true
---
# Realms of Power: Faerie

Pilot `Realms of Power` landing page aligned to the PDF currently present in `~/Ars-Magica/_books/`.

## Status

This title is configured for chapter-level ingestion from `~/Ars-Magica/_books/[AM5e]Realms_of_Power-Faerie.pdf`.

## Chapters

- [01 Realm Overview](./01-realm-overview/)

## Errata

- [View errata layer](../errata/realms-of-power-faerie/)
